...
export async function getUsageStatus({ ip, token, userId, toolKey, date = new Date() }) {
  const plan = (await checkPremiumStatus(ip, token)) ? "premium" : "standard";
  const config = await getToolsConfig();
  const limit = plan === "premium" ? null : config.plans?.standard?.monthlyLimit ?? 3;
  const monthKey = getMonthKey(date);
...
export async function getPlanForClient({ ip, token }) {
  return (await checkPremiumStatus(ip, token)) ? "premium" : "standard";
}
